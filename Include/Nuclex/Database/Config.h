...
#ifndef NUCLEX_DATABASE_CONFIG_H
#define NUCLEX_DATABASE_CONFIG_H

// --------------------------------------------------------------------------------------------- //

// Database recognition
#if defined(WINAPI_FAMILY) && (WINAPI_FAMILY == WINAPI_FAMILY_APP)
  #error The Nuclex.Database.Native library does not support WinRT
#elif defined(WIN32) || defined(_WIN32)
  #define NUCLEX_DATABASE_WINDOWS 1
#else
  #define NUCLEX_DATABASE_LINUX 1
#endif
...
// Compiler support checking
    ...
    #error At least Visual Studio 2019 is required to compile Nuclex.Database.Native
  #elif defined(_MSVC_LANG) && (_MSVC_LANG >= 202002L)
    #define NUCLEX_DATABASE_CXX20 1
  #endif
#elif defined(__clang__) && defined(__clang_major__)
  #if (__clang_major__ < 12) // clang 12.0 has the C++20 features we use
    #error At least clang 12.0 is required to compile Nuclex.Database.Native
  #elif defined(__cplusplus) && (__cplusplus >= 202002L)
    #define NUCLEX_DATABASE_CXX20 1
  #endif
#elif defined(__GNUC__)
  #if (__GNUC__ < 11) // GCC 11.0 has the C++20 features we use
    #error At least GCC 11.0 is required to compile Nuclex.Database.Native
  #elif defined(__cplusplus) && (__cplusplus >= 202002L)
    #define NUCLEX_DATABASE_CXX20 1
  #endif
#else
  #error Unknown compiler. Nuclex.Database.Native is tested with GCC, clang and MSVC only
#endif
...
#if !defined(NUCLEX_DATABASE_CXX20)
  #error The Nuclex.Database.Native library must be compiled in at least C++20 mode
#endif
...
// a dll (Nuclex.Database.Native library). The NUCLEX_DATABASE_SOURCE symbol is defined by
// all source files of the library, so you don't have to worry about a thing.
#if defined(_MSC_VER)

  #if defined(NUCLEX_DATABASE_STATICLIB) || defined(NUCLEX_DATABASE_EXECUTABLE)
    #define NUCLEX_DATABASE_API
  #else
    #if defined(NUCLEX_DATABASE_SOURCE)
      // If we are building the DLL, export the symbols tagged like this
      #define NUCLEX_DATABASE_API __declspec(dllexport)
    #else
      // If we are consuming the DLL, import the symbols tagged like this
      #define NUCLEX_DATABASE_API __declspec(dllimport)
    #endif
  #endif
  ...
  #define NUCLEX_DATABASE_TYPE
    ...
    #define NUCLEX_DATABASE_API __attribute__ ((visibility ("default")))
    #define NUCLEX_DATABASE_TYPE __attribute__ ((visibility ("default")))
  #endif
...
#if defined(NDEBUG)
  #define NUCLEX_DATABASE_NDEBUG_UNUSED(x) (void)x
#else
  #define NUCLEX_DATABASE_NDEBUG_UNUSED(x) {}
#endif

// --------------------------------------------------------------------------------------------- //

#if defined(_MSC_VER)
  #define NUCLEX_DATABASE_CPU_YIELD _mm_pause()
#elif defined(__arm__)
  #define NUCLEX_DATABASE_CPU_YIELD asm volatile("yield")
#else
  #define NUCLEX_DATABASE_CPU_YIELD __builtin_ia32_pause()
#endif

// --------------------------------------------------------------------------------------------- //

#endif // NUCLEX_DATABASE_CONFIG_H
