...
namespace Nuclex::ThinOrm::Errors {
  ...
  class NUCLEX_THINORM_TYPE UnassignedParameterError : public std::invalid_argument {
    ...
    public: NUCLEX_THINORM_API explicit UnassignedParameterError(const std::u8string &message) noexcept;

    /// <summary>Initializes a bad parameter name type error</summary>
    /// <param name="message">Message that describes the error</param>
    public: NUCLEX_THINORM_API explicit UnassignedParameterError(const char8_t *message) noexcept;
